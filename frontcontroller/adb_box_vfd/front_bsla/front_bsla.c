...
static void PT6958_Show(unsigned char DIG1,unsigned char DIG2,unsigned char DIG3,unsigned char DIG4,unsigned char DOT1,unsigned char DOT2,unsigned char DOT3,unsigned char DOT4)
{
	...
	stpio_set_pin(stb, 0 );
 	...
 	PT6958_SendChar(0xc0); //Command 2 address set,(start from 0 ľăÁÁcom1)   11xx????B
    	if(DOT1==1)DIG1=DIG1+0x80;PT6958_SendChar(DIG1);
    	PT6958_SendChar(ICON1); //led power 01-czerwony 02-zielony
    	if(DOT2==1)DIG2=DIG2+0x80;PT6958_SendChar(DIG2);
    	PT6958_SendChar(ICON2); //led zegar
    	if(DOT3==1)DIG3=DIG3+0x80;PT6958_SendChar(DIG3);
    	PT6958_SendChar(ICON3); //led @
    	if(DOT4==1)DIG4=DIG4+0x80;PT6958_SendChar(DIG4);
...
unsigned char  z1,z2,z3,z4,k1,k2,k3,k4;
	...
	k1=0;k2=0;k3=0;k4=0;
	z1=0x20;z2=0x20;z3=0x20;z4=0x20;
	if(len==1) {z4=kbuf[0];}
	if(len==2) {z3=kbuf[0];z4=kbuf[1];}
	if(len==3) {z2=kbuf[0];z3=kbuf[1];z4=kbuf[2];}
	if(len==4) {z1=kbuf[0];z2=kbuf[1];z3=kbuf[2];z4=kbuf[3];}
	if(len==8) 
		{
		z1=kbuf[0];z2=kbuf[1];z3=kbuf[2];z4=kbuf[3];
		if(kbuf[4]==0x31) k1=1;
		if(kbuf[5]==0x31) k2=1;
		if(kbuf[6]==0x31) k3=1;
		if(kbuf[7]==0x31) k4=1;
...
unsigned char poz=0,ico=0;
	...
	if(len==5)
	{
		...
		if (poz==1) {ICON1=ico;poz=0xc1;}
		if (poz==2) {if(ico==1) {ICON1=2;ico=2;} else {ICON1=0;ico=0;} poz=0xc1;}
		if (poz==3) {ICON2=ico;poz=0xc3;}
		if (poz==4) {ICON3=ico;poz=0xc5;}
		if (poz==5) {ICON4=ico;poz=0xc7;}
