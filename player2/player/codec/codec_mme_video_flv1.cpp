...
CodecStatus_t Codec_MmeVideoFlv1_c::FillOutDecodeCommand(void)
{
	...
	DecodeContext->MMEBuffers[FLV1_MME_CURRENT_FRAME_BUFFER_RASTER].ScatterPages_p[0].Page_p = (void *)(void *)BufferState[CurrentDecodeBufferIndex].BufferLumaPointer;
	DecodeContext->MMEBuffers[FLV1_MME_CURRENT_FRAME_BUFFER_RASTER].TotalSize = MaxBytesPerFrame;//((DecodingWidth+32)*(DecodingHeight+32)*3)/2;
	//}}}
