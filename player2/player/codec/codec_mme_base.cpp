	...
	bool              TestForImmediateOutput)
{
	...
	else
	{
		...
		if (BufferState[BufferIndex].DecodesInProgress == 0)
			OutputRing->Insert((uintptr_t)BufferState[BufferIndex].Buffer);
		OS_UnLockMutex(&Lock);
	}
	return CodecNoError;
}
...
#endif
					OutputRing->Insert((uintptr_t)BufferState[Context->BufferIndex].Buffer);
				}
			}
			//
...
CodecStatus_t   Codec_MmeBase_c::TestMarkerFramePassOn(void)
{
			...
			((MMECommandAbortedCount + MMECommandCompletedCount) >= PassOnMarkerBufferAt))
	{
		...
		if (LocalMarkerBuffer)
			OutputRing->Insert((uintptr_t)LocalMarkerBuffer);
	}
	return CodecNoError;
}
