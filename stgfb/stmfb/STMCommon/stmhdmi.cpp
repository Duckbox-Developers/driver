...
#if	defined(SPARK) || defined(SPARK7162) || defined(PACE7241)
    if(hotplugstate == 0)
#else
    if(hotplugstate != 0)
#endif
...
void CSTmHDMI::ProgramV1Config(ULONG &cfg, const stm_mode_line_t *const pModeLine, ULONG tvStandard)
{
  ...
  if((pModeLine->ModeParams.OutputStandards & (STM_OUTPUT_STD_HD_MASK | STM_OUTPUT_STD_NTG5)) == 0)
  {
    if(tvStandard & STM_OUTPUT_STD_TMDS_PIXELREP_2X)
    {
      if(pModeLine->ModeParams.ScanType == SCAN_I)
...
{
  /*
   * Default, do nothing, provided for SoCs that do not have this version of
   * the IP so they do not have to implement an empty function. Code should
   * never get here.
   */
...
bool CSTmHDMI::Start(const stm_mode_line_t* pModeLine, ULONG tvStandard)
{
     ...
     (tvStandard & STM_OUTPUT_STD_CEA861C) == 0)
  {
    ...
    {
      if(((pModeLine->ModeParams.OutputStandards & (STM_OUTPUT_STD_VESA | STM_OUTPUT_STD_NTG5)) == 0) ||
         ((tvStandard & (STM_OUTPUT_STD_VESA | STM_OUTPUT_STD_NTG5)) == 0))
      {
        DEBUGF2(2,("%s: cannot start a non CEA861C,VESA or NTG5 mode on DVI\n",__PRETTY_FUNCTION__));
        return false;
      }
    }
    else
