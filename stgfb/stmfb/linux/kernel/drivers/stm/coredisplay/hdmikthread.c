/***********************************************************************
 *
 * File: linux/kernel/drivers/stm/coredisplay/hdmikthread.c
...
static int stmhdmi_probe(struct i2c_client *client,
...
static int __devexit stmhdmi_remove(struct i2c_client *client)
{
  DPRINTK("%s\n", __FUNCTION__);
...
static const struct i2c_device_id stmhdmi_id[] = {
  { "stmhdmi-eddc", 0 },
  { "stmhdmi-edid", 0 },
  { }
};
MODULE_DEVICE_TABLE(i2c, stmhdmi_id);

static struct i2c_driver stmhdmi_driver = {
   .driver         = {
       .name           = "stmhdmi"
   },
   .probe = stmhdmi_probe,
   .remove = stmhdmi_remove,
   .id_table = stmhdmi_id,
};
...
static struct i2c_board_info stmhdmi_eddc_info = {
        I2C_BOARD_INFO("stmhdmi", I2C_EDDC_SEGMENT_REG_ADDR),
        .type = "stmhdmi-eddc",
        .platform_data = 0,
};

static struct i2c_board_info stmhdmi_edid_info = {
        I2C_BOARD_INFO("stmhdmi", I2C_EDID_ADDR),
        .type = "stmhdmi-edid",
        .platform_data = 0,
};
...
static void stmhdmi_i2c_connect(struct stm_hdmi *hdmi)
{
  ...
  hdmi->eddc_segment_reg_client = i2c_new_device(hdmi->i2c_adapter,
                                                 &stmhdmi_eddc_info);
  hdmi->edid_client = i2c_new_device(hdmi->i2c_adapter,
                                     &stmhdmi_edid_info);
}
