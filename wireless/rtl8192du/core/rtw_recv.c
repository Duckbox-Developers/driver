...
sint On_TDLS_Setup_Req(_adapter *adapter, union recv_frame *precv_frame)
{
	...
	if(ptdls_sta==NULL ||(ptdls_sta->tdls_sta_state&TDLS_LINKED_STATE) )
	{
		...
		if(ptdls_sta) 
		{
			...
			//
			wfd_offset = 0;
			wfd_offset = rtw_get_wfd_ie( ptr + FIXED_IE + wfd_offset, parsing_length - FIXED_IE - wfd_offset, wfd_ie, &wfd_ielen );
			//	Commented by Albert 20120514
			//	Marvell uses two different WFD IE. One is for peer port, another is for local IP
			//	Broadcom uses 1 WFD IE to contain both these two informations.
			//	But this modification will cause the infinite loop in the following loop.
			//	So, skip it now.
			while( wfd_offset )
			//if ( wfd_offset )
			{
				...
				if ( attr_contentlen )
				{
					...
					DBG_8192C( "[%s] Peer IP = %02u.%02u.%02u.%02u \n", __FUNCTION__, 
