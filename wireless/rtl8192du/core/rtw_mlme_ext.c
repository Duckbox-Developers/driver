				...
				switch( frame_body[ 6 ] )//OUI Subtype
				{
					...
					case P2P_INVIT_REQ:
					{
						...
						if ( (p2p_ie=rtw_get_p2p_ie( frame_body + _PUBLIC_ACTION_IE_OFFSET_, len - _PUBLIC_ACTION_IE_OFFSET_, NULL, &p2p_ielen)) )
						{
							...
							DBG_871X( "[%s] peer BSSID %.2X %.2X %.2X %.2X %.2X %.2X\n", __FUNCTION__,
							...
							issue_p2p_invitation_response( padapter, GetAddr2Ptr(pframe), pwdinfo->inviteresp_info.token, match_result );
						}
...
sint On_TDLS_Dis_Rsp(_adapter *adapter, union recv_frame *precv_frame)
{
	...
	else
	{
		...
		{
			printk("pattrib->RxPWDBAll=%d, pdmpriv->UndecoratedSmoothedPWDB=%d\n", pattrib->RxPWDBAll, UndecoratedSmoothedPWDB);
