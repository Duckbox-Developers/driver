...
void _tdls_alive_timer_phase1_hdl(void *FunctionContext)
{
	...
	printk("issue_tdls_dis_req to check alive\n");
...
void _tdls_alive_timer_phase2_hdl(void *FunctionContext)
{
	...
	{
//		printk("TDLS STA ALIVE\n");
		printk("TDLS STA ALIVE, ptdls_sta->sta_stats.last_rx_pkts:%llu, ptdls_sta->sta_stats.rx_pkts:%llu\n",
	...
	{
		if( !(ptdls_sta->tdls_sta_state & TDLS_ALIVE_STATE) )
			printk("TDLS STA TOO FAR\n");
		if( !(sta_last_rx_pkts(ptdls_sta) + 3 <= sta_rx_pkts(ptdls_sta)))
			printk("TDLS LINK WITH LOW TRAFFIC, ptdls_sta->sta_stats.last_rx_pkts:%llu, ptdls_sta->sta_stats.rx_pkts:%llu\n",
				sta_last_rx_pkts(ptdls_sta), sta_rx_pkts(ptdls_sta));
