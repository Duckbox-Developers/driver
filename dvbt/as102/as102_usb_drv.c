...
static struct usb_device_id as102_usb_id_table[] = {
	...
	{ USB_DEVICE(ADB_BOX_USB_VID, ADB_BOX_USB_PID) },
...
/*
	usb_fill_bulk_urb(urb,
			  ...
			  usb_rcvbulkpipe(dev->bus_adap.usb_dev, 2),
			  urb->transfer_buffer,
			  AS102_USB_BUF_SIZE,
...
*/
	...
	pipe = usb_rcvbulkpipe(dev->bus_adap.usb_dev, 2);
	...
	ret = usb_clear_halt(dev->bus_adap.usb_dev, pipe);
	if(ret < 0) {
		dprintk(debug,"error %i in %s during pipe reset\n",ret,__FUNCTION__);
		//return ret;
	}
	else {
		dprintk(debug,"pipe resetted\n");
	} 
	
	err = usb_submit_urb(urb, GFP_ATOMIC);	//org
...
void show_buf(const u8 *buf,u16 len)
{
	u16 len_ = 0;
		dprintk(debug, "LEN:%02X>",len);
		while(len_ < len)
		{
			dprintk(debug, "%02X",buf[len_]);
			len_++;
		}
	dprintk(debug, "\n");
}
...
//512 byte
//LEN:200>

extern void extern_inject_data(u32 *data, off_t size);
//extern void extern_inject_data_fdma(u32 *data, off_t size);
...
#endif
{
	...
	if(urb->status != 0){
		dprintk(debug, "urb->status!=0\n");
	}
	else
	if (urb->actual_length > 0) {
		//show_buf(urb->transfer_buffer,urb->actual_length);
		
		extern_inject_data(urb->transfer_buffer, urb->actual_length);
//		extern_inject_data_fdma(urb->transfer_buffer, urb->actual_length);
...
static int as102_alloc_usb_stream_buffer(struct as102_dev_t *dev)
{
	...
	if(urb == NULL) {
		dprintk(debug, "%s: err usb_alloc_urb\n", __func__);
		return -ENOMEM;
	}  

	urb->transfer_buffer = kzalloc(AS102_USB_BUF_SIZE, GFP_KERNEL);
	if(urb->transfer_buffer == NULL) {
		usb_free_urb(urb);
		dprintk(debug, "%s: err kzalloc\n", __func__);
		return -ENOMEM;
	} 

	urb->transfer_buffer_length = AS102_USB_BUF_SIZE;
	dev->stream_urb[0] = urb;
	LEAVE();
	return ret;
#else
//============== new ===================
	...
	urb = usb_alloc_urb(0, GFP_ATOMIC);	//org
	//urb = usb_alloc_urb(0, GFP_KERNEL);
	if(urb == NULL) {
		dprintk(debug, "%s: err usb_alloc_urb\n", __func__);
		return -ENOMEM;
...
#ifdef URB_SINGLE
	ENTER();
		ret = as102_submit_urb_stream(dev, dev->stream_urb[0]);
		if (ret) {
