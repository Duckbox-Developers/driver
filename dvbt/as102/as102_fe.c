...
static int as102_fe_read_status(struct dvb_frontend *fe, fe_status_t *status)
{
	...
	case TUNE_STATUS_SIGNAL_DVB_OK:
		//dprintk(debug,"TUNE_STATUS_SIGNAL_DVB_OK\n");
		*status = FE_HAS_SIGNAL | FE_HAS_CARRIER;
		break;
	case TUNE_STATUS_STREAM_DETECTED:
		//dprintk(debug,"TUNE_STATUS_STREAM_DETECTED\n");
		*status = FE_HAS_SIGNAL | FE_HAS_CARRIER | FE_HAS_SYNC;
		break;
	case TUNE_STATUS_STREAM_TUNED:
		//dprintk(debug,"TUNE_STATUS_STREAM_TUNED\n");
		*status = FE_HAS_SIGNAL | FE_HAS_CARRIER | FE_HAS_SYNC |
			FE_HAS_LOCK;
		break;
	default:
		*status = TUNE_STATUS_NOT_TUNED;
		//dprintk(debug,"TUNE_STATUS_NOT_TUNED\n");
		}
					 ...
					 u16 *strength)
{
	...
	*strength = (((0xffff * 400) * dev->signal_strength + 41000) * 2);
	//strength_ = (((0xffff * 400) * dev->signal_strength + 41000) * 2);
	//*strength = strength_;
	//dprintk(debug,"STRENGTH:%d\n",strength_);
	dprintk(debug,"STRENGTH:%d\n",*strength);
...
static int as102_get_property(struct dvb_frontend *fe, struct dtv_property* tvp)
{
...
static struct dvb_frontend_ops as102_fe_ops = {
	...
	.get_property		= as102_get_property,
...
static uint8_t as102_fe_get_code_rate(fe_code_rate_t arg)
{
	...
	switch (arg) {
	case FEC_1_2:
		dprintk(debug, "FEC_1_2\n");
		c = CODE_RATE_1_2;
		break;
	case FEC_2_3:
		dprintk(debug, "FEC_2_3\n");
		c = CODE_RATE_2_3;
		break;
	case FEC_3_4:
		dprintk(debug, "FEC_3_4\n");
		c = CODE_RATE_3_4;
		break;
	case FEC_5_6:
		dprintk(debug, "FEC_5_6\n");
		c = CODE_RATE_5_6;
		break;
	case FEC_7_8:
		dprintk(debug, "FEC_7_8\n");
		c = CODE_RATE_7_8;
		break;
	default:
		dprintk(debug, "FEC_UNKOWN\n");
		c = CODE_RATE_UNKNOWN;
		break;
	}
			  ...
			  struct dvb_frontend_parameters *params)
{
	...
	case BANDWIDTH_8_MHZ:
		dprintk(debug, "BW_8_MHZ\n");
		tune_args->bandwidth = BW_8_MHZ;
		break;
	case BANDWIDTH_7_MHZ:
		dprintk(debug, "BW_7_MHZ\n");
		tune_args->bandwidth = BW_7_MHZ;
		break;
	case BANDWIDTH_6_MHZ:
		dprintk(debug, "BW_6_MHZ\n");
	...
	case GUARD_INTERVAL_1_32:
		dprintk(debug, "GUARD_INT_1_32\n");
		tune_args->guard_interval = GUARD_INT_1_32;
		break;
	case GUARD_INTERVAL_1_16:
		dprintk(debug, "GUARD_INT_1_16\n");
		tune_args->guard_interval = GUARD_INT_1_16;
		break;
	case GUARD_INTERVAL_1_8:
		dprintk(debug, "GUARD_INT_1_8\n");
		tune_args->guard_interval = GUARD_INT_1_8;
		break;
	case GUARD_INTERVAL_1_4:
		dprintk(debug, "GUARD_INT_1_4\n");
		tune_args->guard_interval = GUARD_INT_1_4;
		break;
	case GUARD_INTERVAL_AUTO:
	default:
		dprintk(debug, "GUARD_UNKOWN\n");
	...
	case TRANSMISSION_MODE_2K:
		dprintk(debug, "TRANS_MODE_2K\n");
		tune_args->transmission_mode = TRANS_MODE_2K;
		break;
	case TRANSMISSION_MODE_8K:
		dprintk(debug, "TRANS_MODE_8K\n");
		tune_args->transmission_mode = TRANS_MODE_8K;
		break;
	default:
		dprintk(debug, "TRANS_MODE_UNKOWN\n");
		tune_args->transmission_mode = TRANS_MODE_UNKNOWN;
	}

	switch (params->u.ofdm.hierarchy_information) {
	case HIERARCHY_NONE:
		dprintk(debug, "HIER_NONE\n");
		tune_args->hierarchy = HIER_NONE;
		break;
	case HIERARCHY_1:
		dprintk(debug, "HIER_ALPHA_1\n");
		tune_args->hierarchy = HIER_ALPHA_1;
		break;
	case HIERARCHY_2:
		dprintk(debug, "HIER_ALPHA_2\n");
		tune_args->hierarchy = HIER_ALPHA_2;
		break;
	case HIERARCHY_4:
		dprintk(debug, "HIER_ALPHA_4\n");
		tune_args->hierarchy = HIER_ALPHA_4;
		break;
	case HIERARCHY_AUTO:
		dprintk(debug, "HIER_UNKOWN\n");
