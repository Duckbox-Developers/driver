...
static unsigned char first_init = 10;
...
static struct stv090x_reg stv090x_defval[] = {
	...
	{ STV090x_P2_ERRCTRL2,   	0x12 },//0xc1 }, /* P2_ERRCTRL2 */
	...
	{ STV090x_P1_ERRCTRL2,   	0x12 },//0xc1 }, /* P1_ERRCTRL2 */
...
//#define SET_STAPI	//fix 18.11.2012 (SagemCom ESI88)
...
#if defined(SET_STAPI)	//ustawienia z stapi dla stv6110
...
#endif
#if defined(SET_STAPI)	//ustawienia z stapi dla stv6110
	{ STV090x_P2_TNRCFG,		0x4c },
	{ STV090x_P2_TNRCFG2,		0x86 },
	{ STV090x_P2_TNRCFG3,		0x18 },
	{ STV090x_P2_TNRSTEPS,		0x05 },
	{ STV090x_P2_TNRGAIN,		0x41 },
	{ STV090x_P2_TNRADJ,		0x00 },
	{ STV090x_P2_TNRCTL2,		0x97 },
#endif
	{ STV090x_P2_LDT,			0xd0 },
...
#endif
#if defined(SET_STAPI)	//ustawienia z stapi dla stv6110
	{ STV090x_P1_TNRCFG,		0x4c },
	{ STV090x_P1_TNRCFG2,		0x06 },
	{ STV090x_P1_TNRCFG3,		0x18 },
	{ STV090x_P1_TNRSTEPS,		0x05 },
	{ STV090x_P1_TNRGAIN,		0x41 },
	{ STV090x_P1_TNRADJ,		0x00 },
	{ STV090x_P1_TNRCTL2,		0x97 },
#endif
	{ STV090x_P1_LDT,			0xd0 },
...
//default after reset - all GPIO set LOW
	...
	//wszystkie 02 - 1-0
	//wszystkie 82 - 1-1
	//wszystkie 02 + gpio2=82 - 1-1
	//wszystkie 82 + gpio2=02 - 1-0
...
if (state->demod == STV090x_DEMODULATOR_0) 
	dprintk(FE_DEBUG, 1,"!!!!!!! STV090x_DEMODULATOR_0 gate:\n");
else
	dprintk(FE_DEBUG, 1,"!!!!!!! STV090x_DEMODULATOR_1 gate:\n");
...
static enum stv090x_signal_state stv090x_algo(struct stv090x_state *state)
{
	...
	dprintk(FE_DEBUG, 1,">> stv090x_algo\n");
...
//	}
	...
	if ((lock) && (signal_state == STV090x_RANGEOK)) { /* signal within Range */
		stv090x_optimize_track(state);
		if (state->internal->dev_ver <= 0x11) { /*workaround for dual DVBS1 cut 1.1 and 1.0 only*/
...
//			*per = 1;
	...
	if (STV090x_WRITE_DEMOD(state, FBERCPT4, 0) < 0) goto err;
	if (STV090x_WRITE_DEMOD(state, ERRCTRL2, 0x12) < 0)	goto err;
//	if (STV090x_WRITE_DEMOD(state, ERRCTRL2, 0xc1) < 0)
	//if (STV090x_WRITE_DEMOD(state, ERRCTRL2, 0x11) < 0)	goto err;
...
if (state->demod == STV090x_DEMODULATOR_0) 
	dprintk(FE_DEBUG, 1,"!!!!!!! STV090x_DEMODULATOR_0 set_tone:");
else
	dprintk(FE_DEBUG, 1,"!!!!!!! STV090x_DEMODULATOR_1 set_tone:");


	reg = STV090x_READ_DEMOD(state, DISTXCTL);
	switch (tone) {
	case SEC_TONE_ON:
			dprintk(FE_DEBUG, 1,"TONE_ON\n");
...
static int stv090x_set_voltage(struct dvb_frontend *fe, fe_sec_voltage_t voltage)
{
	...
	u8 b;
	int ret;
	struct i2c_msg msg = {.addr = 0x0b, .flags = 0,
				.buf = &b,
				.len = 1 };

if (state->demod == STV090x_DEMODULATOR_0) 
	printk("!!!!!!! STV090x_DEMODULATOR_0 set_voltage:");
else
	printk("!!!!!!! STV090x_DEMODULATOR_1 set_voltage:");

#if 0
		b=0x34;//lnb1	34	3b
		ret=i2c_transfer(state->i2c, &msg, 1);
		if (ret=!1) printk("SEC_VOLTAGE_13 Error A8292\n");
		b=0x34+64;//lnb2	34	3b
		ret=i2c_transfer(state->i2c, &msg, 1);
		if (ret=!1) printk("SEC_VOLTAGE_13 Error A8292\n");
	...
	case SEC_VOLTAGE_18:
		...
		b=0x3b;//lnb1	34	3b
		ret=i2c_transfer(state->i2c, &msg, 1);
		if (ret=!1) printk("SEC_VOLTAGE_18 Error A8292\n");
		}
		else{
		b=0x3b+64;//lnb2	34	3b
		ret=i2c_transfer(state->i2c, &msg, 1);
		if (ret=!1) printk("SEC_VOLTAGE_18 Error A8292\n");
		}
		break;
	case SEC_VOLTAGE_OFF:
		...
		b=0x10;//lnb1	34	3b
		ret=i2c_transfer(state->i2c, &msg, 1);
		if (ret=!1) printk("SEC_VOLTAGE_OFF Error A8292\n");
		}
		else{
		b=0x10+64;//lnb2	34	3b
		ret=i2c_transfer(state->i2c, &msg, 1);
		if (ret=!1) printk("SEC_VOLTAGE_OFF Error A8292\n");
		}
		break;
...
static int stv090x_set_tspath(struct stv090x_state *state)
{
	...
	//fix dla cut3.0 - 18.11.2012 (SagemCom ESI88)
	if (state->internal->dev_ver >= 0x30) {
		if((state->config->ts1_mode==STV090x_TSMODE_SERIAL_CONTINUOUS)&&(state->config->ts2_mode==STV090x_TSMODE_SERIAL_CONTINUOUS))
			stv090x_write_reg(state, STV090x_TSGENERAL, 0x00);
		else
			stv090x_write_reg(state, STV090x_TSGENERAL, 0x40);
	}
	else if (state->internal->dev_ver >= 0x20) {
...
static int stv090x_init(struct dvb_frontend *fe)
{
	...
	struct i2c_msg msg = {.addr = 0x0b, .flags = 0,
				.buf = &b,
				.len = 1 };
	struct i2c_msg msg_r = {.addr = 0x0b, I2C_M_RD,
				.buf = &b,
				.len = 1 };
	...
	ret=i2c_transfer(state->i2c, &msg_r, 1);
	if (ret=!1) printk("Error:LNB init1 A8292\n");

	b=0x10;//lnb1
	ret=i2c_transfer(state->i2c, &msg, 1);
	if (ret=!1) printk("Error:LNB1 init2 A8292\n");

	b=0x10+64;//lnb2
	ret=i2c_transfer(state->i2c, &msg, 1);
	if (ret=!1) printk("Error:LNB2 init3 A8292\n");

	b=0x82;//lnb1
	ret=i2c_transfer(state->i2c, &msg, 1);
	if (ret=!1) printk("Error:LNB1 init4 A8292\n");

	b=0x82+64;//lnb2
	ret=i2c_transfer(state->i2c, &msg, 1);
	if (ret=!1) printk("Error:LNB2 init5 A8292\n");
...
static int stv090x_setup(struct dvb_frontend *fe)
{
	...
	dprintk(FE_DEBUG, 1,">> stv090x_setup\n");
	...
	/* Set No Tuner Mode */
	dprintk(FE_DEBUG, 1,"Set No Tuner Mode\n");
	...
	/* I2C repeater OFF */
	dprintk(FE_DEBUG, 1,"I2C repeater OFF\n");
...
//62 - frontend1
	...
	//40-off c0-on
	//reg=0xc0;	//otwiera gate dla addr=0x62 i jest to frontend1
	printk("stv090x_write_reg(state, STV090x_P1_I2CRPT, %04x)\n",reg);
	if (stv090x_write_reg(state, STV090x_P1_I2CRPT, reg) < 0) //frontend1
		goto err;
	//reg=0xc0;
	printk("stv090x_write_reg(state, STV090x_P1_I2CRPT, %04x)\n",reg);
	if (stv090x_write_reg(state, STV090x_P2_I2CRPT, reg) < 0) //frontend0
		goto err;

	printk("stv090x_write_reg(state, STV090x_NCOARSE, 0x13)\n");
	if (stv090x_write_reg(state, STV090x_NCOARSE, 0x13) < 0) /* set PLL divider */
		goto err;
	msleep(5);
	printk("stv090x_write_reg(state, STV090x_I2CCFG, 0x08)\n");
	if (stv090x_write_reg(state, STV090x_I2CCFG, 0x08) < 0) /* 1/41 oversampling */
		goto err;
	printk("stv090x_write_reg(state, STV090x_SYNTCTRL, 0x20)\n");
...
//		state->frontend.ops			= stv090x_ops_2;
	...
	if (state->demod == STV090x_DEMODULATOR_0) {mutex_init(&demod_lock);mutex_init(&tuner_lock);}
