...
/* Gives access to the output(s) -> analogue, hdmi, virtual, etc. */
...
typedef enum
{
  ...
  STVTG_TIMING_MODE_540P29970_18000,       /* NTG5: QFHD1830,  Quarter HD 30Hz */
  STVTG_TIMING_MODE_540P25000_18000,       /* NTG5: QFHD1825,  Quarter HD 25Hz */
  STVTG_TIMING_MODE_540P59940_36343,       /* NTG5: QFHD3660,  Quarter HD 60Hz */
  STVTG_TIMING_MODE_540P49993_36343,       /* NTG5: QFHD3650,  Quarter HD 50Hz */
  STVTG_TIMING_MODE_540P59945_54857_WIDE,  /* NTG5: WQFHD5660, 1440x540 60Hz   */
  STVTG_TIMING_MODE_540P49999_54857_WIDE,  /* NTG5: WQFHD5650, 1440x540 50Hz   */
  STVTG_TIMING_MODE_540P59945_54857,       /* NTG5: QFHD5660,  Quarter HD 60Hz */
  STVTG_TIMING_MODE_540P49999_54857,       /* NTG5: QFHD5650,  Quarter HD 50Hz */
...
#define STM_OUTPUT_STD_NTG5          (1L<<10)
...
#define STM_CTRL_CAPS_CGMS               (1L<<5)
#define STM_CTRL_CAPS_EXTERNAL_SYNC      (1L<<6)
...
typedef enum
{
   ...
   */
  STM_CTRL_MAX_PIXEL_CLOCK=1,    /* Implied by STM_OUTPUT_CAPS_MODE_CHANGE */
  STM_CTRL_CLOCK_ADJUSTMENT,     /* Implied by STM_OUTPUT_CAPS_MODE_CHANGE */
  ...
  STM_CTRL_EXTERNAL_SYNC_SHIFT,         /* STM_CTRL_CAPS_EXTERNAL_SYNC     */
  STM_CTRL_EXTERNAL_SYNC_INVERT,        /* STM_CTRL_CAPS_EXTERNAL_SYNC     */

  /*
   * Analogue output controls
   */
  STM_CTRL_BRIGHTNESS,              /* STM_CTRL_CAPS_BRIGHTNESS        */
  ...
  STM_CTRL_CVBS_TRAP_FILTER,     /* Implied by STM_OUTPUT_CAPS_SD_ANALOGUE */
  STM_CTRL_CHROMA_SCALE,         /* Implied by STM_OUTPUT_CAPS_SD_ANALOGUE */
  ...
  /*
   * (F)DVO output controls
   */
  STM_CTRL_FDVO_INVERT_DATA_CLOCK,      /* STM_CTRL_CAPS_FDVO_CONTROL      */
  STM_CTRL_FDVO_ALLOW_EMBEDDED_SYNCS,   /* STM_CTRL_CAPS_FDVO_CONTROL      */

  /*
   * Blit compositor virtual output controls
   */
 ...
 *
 * Get the OR'd capabilities of the given output (STM_OUTPUT_CAPS_MODE_CHANGE,
 * etc.).
 *
 * Output capabilities include, but are not limited to:
 *   - Analogue SD/ED/HD
 *   - DVO/HDMI
 *   - Virtual output (blitter composition)
 *   - Mode change
 *   - Mixer
 *   - Signals (Y/C, RGB, CVBS etc.)
 *
 ...
 *
 * Control capabilities include, but are not limited to:
 *   - Background colour
 *   - Brightness (STM_CTRL_CAPS_BRIGHTNESS)
 *   - Saturation (STM_CTRL_CAPS_SATURATION)
 *   - Contrast (STM_CTRL_CAPS_CONTRAST)
 *   - Hue (STM_CTRL_CAPS_HUE)
 *   - AVmute (STM_CTRL_CAPS_AVMUTE)
 *   - DAC voltage (STM_CTRL_CAPS_DAC_VOLTAGE)
 *   - Signal range (STM_CTRL_CAPS_SIGNAL_RANGE)
 *
 * Returns: -1 if the device lock cannot be obtained, otherwise it returns 0.
 *
 */
 ...
 *                                    ULONG newVal)
 *
 * Set the specified control to the given value.
 *
 * Returns: -1 if the device lock cannot be obtained, otherwise it returns 0.
 *
 * Note: If the control is not supported by the output then the request
 * is simply ignored, i.e. that is not considered an error.
 * If an attempt to set an invalid value to an existing control is made,
 * the request is again ignored and no error returned.
 ...
 *                                    ULONG *ctrlVal)
 *
 * Get the value of the specified control.
 *
 * Returns: -1 if the device lock cannot be obtained, otherwise it returns 0.
 *
 * Note: If the control is not supported by the output then ctrlVal
 * will be set to 0, i.e. that is not considered an error.
 *
 */
 ...
 *
 * Mode identifiers include, but are not limited to:
 *   - NTSC
 *   - PAL
 *   - SMPTE 274M
 *   - SMPTE 296M
 ...
 *
 * Returns: NULL if the device lock cannot be obtained or if the output is
 * currently stopped.
 *
 */
 ...
 *                                                ULONG *standard)
 *
 * Query the current TV standard.
 *
 * This is only really relevant for SD interlaced modes which can have
 * different encodings, e.g. PAL/SECAM, NTSC/NTSC-J etc.. If the output
 * is stopped then the standard will be set to zero.
 * Standard includes, but is not limited to:
 *   - STM_OUTPUT_STD_PAL_BDGHI
 *   - STM_OUTPUT_STD_PAL_M
 *   - STM_OUTPUT_STD_NTSC_M
 *
 * Returns: -1 if the device lock cannot be obtained, otherwise it returns 0.
 ...
 *                              ULONG tvStandard)
 *
 * Start the given output with the specified display mode.
 *
 * For an output that supports mode change (see
 * stm_display_output_get_capabilities()) this can be any supported display
 * mode.
 * For outputs that are slaved off a master's timing generator, such as HDMI
 * or DVO, the mode passed to start must be identical to the master's mode
 * (which must already have been started).
 * tvStandard includes, but is not limited to:
 *   - STM_OUTPUT_STD_PAL_BDGHI
 *   - STM_OUTPUT_STD_PAL_M
 *   - STM_OUTPUT_STD_NTSC_M
 *
 ...
 *
 * Stop the given output such that it no longer produces any display.
 *
 * This will fail if any planes are still connected to the output.
 *
 ...
 *
 * Suspend the given output.
 *
 * Used as part of power management operations. It will do whatever is
 * necessary to powerdown the physical outputs and to prevent the hardware
 * accessing memory so that the overall power management system can safely
 * put the DRAM into self refresh.
 *
 * Returns: -1 if the device lock cannot be obtained, otherwise it returns 0.
 *
 * Note: This does not implement hardware state saving to memory for
 * "suspend to disc".
 * It is assumed that the chip continues to be powered sufficiently
 * to maintain the hardware registers in the suspended state.
 *
 */
 ...
 *
 * Resume the given output from a suspended state.
 *
 * Used as part of power management operations.
 *
 * Returns: -1 if the device lock cannot be obtained, otherwise it returns 0.
 *
 */
 ...
 *
 * Queue the metadata structure d on the output for presentation.
 *
 * For more details see include/stmmetadata.h
 *
 * Returns: -1 if the device lock cannot be obtained, otherwise it returns 0.
 ...
 *
 * Enable the output's mixer background, if it has one.
 *
 * The default state of the output's background is implementation dependent.
 *
 * Returns: -1 if the device lock cannot be obtained, otherwise it returns 0.
 *
 */
 ...
 *
 * Disable the output's mixer background, if it has one.
 *
 * The default state of the output's background is implementation dependent.
 *
 * Returns: -1 if the device lock cannot be obtained, otherwise it returns 0.
 ...
 *
 * For master outputs that support mode change this will at least include
 * the associated VSync interrupt. However it may include other interrupts,
 * such as those required by HDMI.
 *
 */
 ...
 * in interlaced modes and the time interval between the last two VSyncs.
 *
 ...
 *
 * Get the status of the output to the physical display.
 *
 * Analogue outputs always return STM_DISPLAY_CONNECTED as there is no
 * way of telling if a display is connected or not.
 * An HDMI output will change its state based on changes to the hot plug
 * pin on the HDMI connector; when that is connected to the HDMI block's
 * interrupts in the chip.
 *
 * This can be called from interrupt context, therefore it neither takes
 * any locks nor does it return errors.
 *
 ...
 *                                    stm_display_status_t status)
 *
 * Set the status of the output to the physical display.
 *
 * Used to inform an output about a platform specific change to the
 * output status such as PIO polled HDMI hotplug events.
 *
 * This can be called from interrupt context, therefore it neither takes
 * any locks nor does it return errors.
 *
 ...
 *
 * This is currently defined for master outputs that own a video timing
 * generator, where the timing generator will be reset and will immediately
 * issue a top field vsync.
 *
